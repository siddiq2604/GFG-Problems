...
class Solution
{
    ...
    private static boolean isDeadEndUtil(Node root, int low, int high) {
        if (root == null) {
            return false;
        }
        if (low == high) {
        ...
        boolean leftDeadEnd = isDeadEndUtil(root.left, low, root.data - 1);
        boolean rightDeadEnd = isDeadEndUtil(root.right, root.data + 1, high);
